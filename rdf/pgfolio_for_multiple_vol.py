...
def check_haspart(g, pg_id):
    new_info = {}
    loc_info = {}
    locations = g.objects(BDR[pg_id], BDO["contentLocation"])
    try:
        ...
        print("not wotking")
    pg_ids = g.objects(BDR[pg_id], BDO["hasPart"])
    print(pg_ids)
    return loc_info, pg_ids
...
def get_pg_ids_without_haspart(g, _id):
    ...
    for pg in pgs:
        pg_id = get_id(str(pg))
    get_pg_ids_without_haspart(g, pg_id)
    print(f"No hasPart for {_id}")
    pg_ids.append(_id)
    print(pg_ids)
    return pg_ids

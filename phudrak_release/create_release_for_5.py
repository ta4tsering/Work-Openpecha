...
def erase_page_index(line):
    new_line = ""
    if re.search(r"image|Image", line):
        lines = re.split(r"image|Image", line )
        for num, line in enumerate(lines, 1):
            if num == 2:
                if line != '':
                    ...
                    if page_side == None:
                        if re.search(r"\-", line):
                            imgnums = re.split(r"\-", line)
                            for num, imgnum in enumerate(imgnums, 1):
                                if num == 1:
                                    img_index = int(imgnum)
                        else:
                            img_index = int(line)
                    else:
                        image_groups = re.split(r"a|b|A|B", line )
                        for img_num, image_group in enumerate(image_groups, 1):
                            if img_num == 1:
                                if image_group != '':
                                    page_num = int(image_group)    
                            if page_num == 1:
                                img_index = page_num
                            if page_side == "a":
                                img_index = int((page_num)+(page_num - 1))
                            elif page_side == "b":
                                img_index = int((page_num)+(page_num))
        new_line += f" i-{img_index}"
    return new_line
...
def get_clean_pages(lines):
    ...
    for num, line in enumerate(lines,1):
        if  num % 2 != 0:
            ...
            if re.search(r"image|Image", new_line):
                new_line = erase_page_index(new_line)
...
def change_file_name_pages(pages, pages_path, meta_info, pagination_path):
    _type = "pages"
    for num, vol in enumerate(pages,1):
        ...
        for info_num, meta in meta_info.items():
            ...
            if new_vol == vol_num:
                ...
                write_text(pages_content, pages_path, vol_num, volume_title, _type)
        ...
        if file.endswith(".txt"):
            ...
            for info_num, meta in meta_info.items():
                ...
                if new_vol == file_name:
                    ...
                    write_text(content, new_path, vol_num, volume_title, _type)    
                    break
...
if __name__=="__main__":
    ...
    change_file_name_pages(pages, pages_path, meta_info, pagination_path)
